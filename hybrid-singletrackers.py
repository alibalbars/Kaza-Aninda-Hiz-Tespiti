...
def main():
    ...
    while True:
        ...
        # sadece üç kere çalışır
        if counter == 1 or isBboxesFilled == False:
            bboxes = get_bboxes(img) # yolo
            # p.print("bboxes\n" + str(bboxes))

            # List'in içi dolu (# sadece bir kere çalışır)
            if bboxes:
                isBboxesFilled = True
                for bbox in bboxes:
                    tracker = cv2.TrackerMOSSE_create()
        ...
        # multitracker'ın takip ettiği araçların konumlarının yenilenmesi
        success, tboxes = multiTracker.update(img)

        # tbox'ları ekrana yazdır (ilk karede doğru çiziyor fakat)
        for i, tbox in enumerate(tboxes): #tbox x1,y1,x2,y2
            p1 = (int(tbox[0]), int(tbox[1]))
            p2 = (int(tbox[2]), int(tbox[3]))
            cv2.rectangle(img, p1, p2, (204, 235, 52), 2) # p1-tl p2-br
